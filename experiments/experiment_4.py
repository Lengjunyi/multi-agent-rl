...
models = [AdvantageMARL().to(device) for _ in range(N)]
advantage = [AdvantageMARL().to(device) for _ in range(N)]

model_data = torch.load("saved_models\\advantage_100eps_double_q.pth")

for i in range(N):
    # load parameters
    models[i].load_state_dict(model_data[f"model_{i%2+1}"])
    advantage[i].load_state_dict(model_data[f"advantage_model_{i%2+1}"])
